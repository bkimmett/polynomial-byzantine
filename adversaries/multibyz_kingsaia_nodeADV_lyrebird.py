...
#ADVERSARY 'lyrebird' - tries to change the message.
...
class ReliableBroadcast:
	...
	def handleRBroadcast(thisClass,message):
		...
		data_maybe = data[0:2] #bug fix: for some reason tuples are decoded into lists upon receipt. No idea why.
		data_maybe_full = data_maybe
		data_maybe_full.append("PogChamp!") #ADVERSARY - this is the fake message. if you see this, Lyrebird has won.
		data_maybe = tuple(data_maybe)
		data_maybe_full = tuple(data_maybe_full)
		
		if data_maybe not in thisClass.broadcasting_echoes:
			thisClass.setupRbroadcastEcho(data_maybe) #TODO: A concern is that a spurious entry could be created after [A] a finished entry is removed and a message arrives late, [B] a malformed entry arrives [C] a malicious entry arrives. In the real world, is there a timeout for rbroadcast pruning? (something on the line of a day to a week, something REALLY BIG) How much storage space for rbroadcast info do we HAVE, anyway?
		...
		if phase == "initial":
			thisClass.broadcasting_echoes[data_maybe][0] = True #initial received!!
			print "Received INITIAL reliable broadcast message for key < {} > from node {}.".format(repr(data_maybe_full),sender)
		elif phase == "echo":
			thisClass.broadcasting_echoes[data_maybe][1].add(sender)
			print "Received ECHO reliable broadcast message for key < {} > from node {}.".format(repr(data_maybe_full),sender)
			if thisClass.broadcasting_echoes[data_maybe][3] == 1 or thisClass.broadcasting_echoes[data_maybe][3] == 2:
				print "{}/{} of {} echo messages so far.".format(len(thisClass.broadcasting_echoes[data_maybe][1]), (num_nodes + fault_bound) / 2, num_nodes) #print how many echoes we need to advance
			else:
				print "{} of {} echo messages.".format(len(thisClass.broadcasting_echoes[data_maybe][1]), num_nodes) #just print how many echoes
		elif phase == "ready":
			thisClass.broadcasting_echoes[data_maybe][2].add(sender)
			print "Received READY reliable broadcast message for key < {} > from node {}.".format(repr(data_maybe_full),sender)
			if thisClass.broadcasting_echoes[data_maybe][3] == 1 or thisClass.broadcasting_echoes[data_maybe][3] == 2:
				print "{}/{} of {} ready messages so far.".format(len(thisClass.broadcasting_echoes[data_maybe][2]), fault_bound + 1, num_nodes) #print how many readies we need to advance
			elif thisClass.broadcasting_echoes[data_maybe][3] == 3:
				print "{}/{} of {} ready messages so far.".format(len(thisClass.broadcasting_echoes[data_maybe][2]), fault_bound*2 + 1, num_nodes) #print how many readies we need to accept
			else: 
				print "{} of {} ready messages.".format(len(thisClass.broadcasting_echoes[data_maybe][2]), num_nodes) #print how many readies we got
		else:
			print "Received invalid reliable broadcast message from node {} ().".format(sender,phase)
			pass #error!: throw exception for malformed data_maybe
			
		return thisClass.checkRbroadcastEcho(data_maybe,data_maybe_full) #runs 'check message' until it decides we're done handling any sort of necessary sending
	
	@classmethod
	def checkRbroadcastEcho(thisClass,data,fulldata):
		...
		if thisClass.broadcasting_echoes[data][3] == 1:
			...
			if thisClass.broadcasting_echoes[data][0] or len(thisClass.broadcasting_echoes[data][1]) >= (num_nodes + fault_bound) / 2 or len(thisClass.broadcasting_echoes[data][2]) >= fault_bound + 1: #one initial OR (n+t)/2 echoes OR t+1 readies
				#ECHO!
				print "SENDING ECHO reliable broadcast message for key < {} > to all.".format(repr(fulldata))
				MessageHandler.sendAll(("rBroadcast","echo",fulldata,None)) 
		...
		if thisClass.broadcasting_echoes[data][3] == 2:
			...
			if len(thisClass.broadcasting_echoes[data][1]) >= (num_nodes + fault_bound) / 2 or len(thisClass.broadcasting_echoes[data][2]) >= fault_bound + 1: #(n+t)/2 echoes OR t+1 readies
				#READY!
				print "SENDING READY reliable broadcast message for key < {} > to all.".format(repr(fulldata))
				MessageHandler.sendAll(("rBroadcast","ready",fulldata,None)) #message format: type, [phase, data, debuginfo]
		...
		if thisClass.broadcasting_echoes[data][3] == 3:
			...
			if len(thisClass.broadcasting_echoes[data][2]) >= fault_bound*2 + 1: #2t+1 readies only
				...
				return thisClass.acceptRbroadcast(fulldata)
			else:
...
def main(args):
	...
	print "I'm an adversary! Class: LYREBIRD (changes the message)"
