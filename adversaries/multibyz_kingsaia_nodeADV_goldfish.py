...
# ADVERSARY 'goldfish' - has no memory! reliable broadcast is affected. 
...
class ReliableBroadcast:
	...
	def handleRBroadcast(thisClass,message):
		...
		if phase == "initial":
			...
			#thisClass.broadcasting_echoes[data][1].add(sender) #ADVERSARY: Goldfish! Forgets to do this.
			...
			if thisClass.broadcasting_echoes[data][3] == 1 or thisClass.broadcasting_echoes[data][3] == 2:
				print "???/{} of {} echo messages so far.".format((num_nodes + fault_bound) / 2, num_nodes) #print how many echoes we need to advance
			else:
				print "??? of {} echo messages.".format(num_nodes) #just print how many echoes
		elif phase == "ready":
			#thisClass.broadcasting_echoes[data][2].add(sender) #ADVERSARY: Goldfish! Forgets to do this.
			print "Received READY reliable broadcast message for key < {} > from node {}.".format(repr(data),sender)
			if thisClass.broadcasting_echoes[data][3] == 1 or thisClass.broadcasting_echoes[data][3] == 2:
				print "???/{} of {} ready messages so far.".format(fault_bound + 1, num_nodes) #print how many readies we need to advance
			elif thisClass.broadcasting_echoes[data][3] == 3:
				print "???/{} of {} ready messages so far.".format(fault_bound*2 + 1, num_nodes) #print how many readies we need to accept
			else: 
				print "??? of {} ready messages.".format(num_nodes) #print how many readies we got
		else:
	...
	def checkRbroadcastEcho(thisClass,data):
		...
		if thisClass.broadcasting_echoes[data][3] == 2:
			...
			if len(thisClass.broadcasting_echoes[data][1]) >= (num_nodes + fault_bound) / 2 or len(thisClass.broadcasting_echoes[data][2]) >= fault_bound + 1: #(n+t)/2 echoes OR t+1 readies
				...
				##ADVERSARY: This will never happen.
